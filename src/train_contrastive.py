...
def load_bm25_cache(jsonl_path: str):
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            cache[nct_id] = rec["candidates"] # keep full info
    return cache
...
def prepare_data_artifacts(args):
    ...
    pairs_train = pd.read_csv(args.train_pair_path, dtype={"nct_id": str, "pubmed_id": str}).fillna("")
    pairs_val   = pd.read_csv(args.val_pair_path, dtype={"nct_id": str, "pubmed_id": str}).fillna("")
    ...
    missing = required_cols - set(pairs_train.columns)
    if missing:
    ...
    print(f"Pairs rows:  {len(pairs_train):,}")
    print(f"Unique nct_id in pairs: {pairs_train['nct_id'].nunique():,}")
    print(f"Pos (label=1): {int((pairs_train['label']==1).sum()):,}")
    print(f"Neg (label=0): {int((pairs_train['label']==0).sum()):,}")
    '''
    ...
    train_nct_ids = set(pairs_train["nct_id"].unique().tolist())
    dump_candidates("train", train_nct_ids)
...
def build_examples_from_pairs(pairs_df, trial_text, doc_text):
    """
    Build SentenceTransformers InputExamples directly from labeled pairs.
    Each row: (trial_text, doc_text, label)
    """
    examples = []
    skipped = 0
    ...
    for _, row in pairs_df.iterrows():
        ...
        if nct_id not in trial_text or pubmed_id not in doc_text:
            skipped += 1
            continue

        examples.append(
            InputExample(
                texts=[trial_text[nct_id], doc_text[pubmed_id]],
                label=label
            )
        )

    print(f">>> [Train] Built {len(examples):,} examples (skipped {skipped:,})")
    return examples
...
def train_sentence_transformer(examples, args):
    ...
    training_args = TrainingArguments(
        ...
        no_cuda=True,  # ← 关键：强制不使用 CUDA/MPS
        use_cpu=True,   # ← 关键：强制使用 CPU
        dataloader_pin_memory=False,  # ← 关键：禁用 pin_memory
    )
...
def main():
    ...
    parser.add_argument("--train_pair_path", default="data/dataset_train.csv")
    parser.add_argument("--val_pair_path", default="data/dataset_valid.csv")
    ...
    parser.add_argument("--hard_k", type=int, default=20)
    parser.add_argument("--n_random", type=int, default=1)
    parser.add_argument("--random_pool_k", type=int, default=200)
    parser.add_argument("--max_labeled_neg", type=int, default=2)
